...
# ── MT5 CONNECTION ─────────────────────────────────────────────────────
def connect():
  # Attach to already open MT5 terminal — no login needed
  ...
  if info:
    ...
    log.error(f"Could not get account info: {mt5.last_error()}")
    return False
...
def get_candles(tf_key: str, count: int = 50):
  ...
  if tf is None:
    log.warning(f"Unknown timeframe: {tf_key}")
    return []
  # Ensure symbol is visible in MT5 market watch
  mt5.symbol_select(SYMBOL, True)
  rates = mt5.copy_rates_from_pos(SYMBOL, tf, 0, count)
  if rates is None or len(rates) == 0:
    log.warning(f"No candle data for {SYMBOL} {tf_key} | MT5 error: {mt5.last_error()}")
    return []
...
# ── MAIN LOOP ──────────────────────────────────────────────────────────
def get_signal_from_tool() -> dict:
  """
  Get signal direction and confidence from our Render tool.
  Then override entry/sl/tp with live MT5 prices — never use tool prices.
  """
  try:
    r = requests.get(f"{ANALYZER_URL}/api/signal", timeout=10)
    if r.status_code == 200:
      data = r.json()
      # Get live price and levels from MT5 — ignore tool prices entirely
      price = get_price()
      if not price:
        log.warning("Cannot get live price from MT5")
        return {"signal": "WAIT", "confidence": 0}
      atr     = calc_atr(get_candles("1h", 50))
      sl_dist = atr * 0.8
      rr      = BASE_RR
      is_long = data.get("is_long", True)
      entry   = price
      sl      = entry - sl_dist if is_long else entry + sl_dist
      tp1     = entry + sl_dist * rr * 0.5 if is_long else entry - sl_dist * rr * 0.5
      tp2     = entry + sl_dist * rr        if is_long else entry - sl_dist * rr
      be      = entry + sl_dist * 0.5       if is_long else entry - sl_dist * 0.5
      # Build result using tool signal + MT5 prices
      result = {
        ...
        "fvg":        data.get("fvg", "?"),
        "in_session": data.get("in_session", True),
      }
      log.info(f"Tool signal: {result['signal']} | Confidence: {result['confidence']}% | MT5 price: {price:.2f}")
      return result
    else:
      log.warning(f"Tool returned {r.status_code} — waiting for valid signal")
      return {"signal": "WAIT", "confidence": 0}
  except Exception as e:
    log.warning(f"Could not reach tool: {e}")
    return {"signal": "WAIT", "confidence": 0}
...
def run_cycle():
  ...
  # Get signal from our Render tool first, fall back to local if unreachable
  result = get_signal_from_tool()

  log.info(
    f"Signal: {result.get(chr(39)+'signal'+chr(39), chr(39)+'UNKNOWN'+chr(39))} | "
    f"Confidence: {result.get(chr(39)+'confidence'+chr(39), 0)}% | "
    f"4H: {result.get(chr(39)+'bias_4h'+chr(39), chr(39)+'?'+chr(39))} | 1H: {result.get(chr(39)+'bias_1h'+chr(39), chr(39)+'?'+chr(39))} | "
    f"Sweep: {result.get(chr(39)+'sweep'+chr(39), chr(39)+'?'+chr(39))} | CHoCH: {result.get(chr(39)+'choch'+chr(39), chr(39)+'?'+chr(39))} | "
    f"FVG: {result.get(chr(39)+'fvg'+chr(39), chr(39)+'?'+chr(39))}"
  )

  # Notify phone if high confidence
  if result.get("confidence", 0) >= MIN_CONFIDENCE:
    notify_server(result)

  # Open trade if criteria met
  if not state.trade_open and result.get("confidence", 0) >= MIN_CONFIDENCE:
    open_trade(result)
...
def main():
  ...
  # Verify we can actually get candle data before starting
  log.info("Checking MT5 data feed...")
  mt5.symbol_select(SYMBOL, True)
  test = mt5.copy_rates_from_pos(SYMBOL, mt5.TIMEFRAME_H1, 0, 5)
  if test is None or len(test) == 0:
    log.error(f"Cannot get candle data from MT5! Error: {mt5.last_error()}")
    log.error("Make sure XAUUSD is visible in MT5 Market Watch (right-click > Show All)")
  else:
    log.info(f"Data feed OK — got {len(test)} candles. Last close: {test[-1][4]:.2f}")
