...
def get_signal_from_tool() -> dict:
  """
  Ask the Render tool for its analysis signal.
  The tool does the full SMC analysis — we just act on the result.
  """
  try:
    r = requests.get(f"{ANALYZER_URL}/api/signal", timeout=10)
    if r.status_code == 200:
      data = r.json()
      log.info(f"Tool signal: {data.get(chr(39)+'signal'+chr(39))} | Confidence: {data.get(chr(39)+'confidence'+chr(39))}%")
      return data
    else:
      log.warning(f"Tool returned status {r.status_code} — falling back to local analysis")
      return analyse()
  except Exception as e:
    log.warning(f"Could not reach tool ({e}) — falling back to local analysis")
    return analyse()
