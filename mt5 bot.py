...
MIN_CONFIDENCE   = 70            # Only trade if analysis ≥ 70%
MAX_DAILY_LOSSES = 3             # Hard stop after 3 losses

# MT5 login (use environment variables in production)
MT5_LOGIN    = 0                 # Your MT5 account number
MT5_PASSWORD = ""                # Your MT5 password
MT5_SERVER   = ""                # Your broker server e.g. "ICMarkets-Demo"

# Analyzer server (your Render URL)
...
def open_trade(result: dict) -> bool:
  ...
  if not result["in_session"]:
    log.info("Outside prime session — skipping")
    return False
...
def run_cycle():
  ...
  if state.daily_losses >= MAX_DAILY_LOSSES:
    log.info("Daily loss limit reached — bot paused until tomorrow")
    return
