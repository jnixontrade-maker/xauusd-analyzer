...
const TWELVE_DATA_KEY = '9b22093d5f6841e09672660351f7f61c';
const TWELVE_SYMBOL  = 'XAU/USD';

async function fetchPrice() {
  const r = await fetch(`https://api.twelvedata.com/price?symbol=${TWELVE_SYMBOL}&apikey=${TWELVE_DATA_KEY}`);
  const d = await r.json();
  if (d.price) return parseFloat(d.price);
  throw new Error('Twelve Data price error: ' + JSON.stringify(d));
}

async function fetchCandles(interval, count) {
  // Twelve Data interval format: 1min, 15min, 1h, 4h, 1day
  const tdInterval = { '1m':'1min', '15m':'15min', '60m':'1h', '1d':'1day' }[interval] || '1h';
  const r = await fetch(`https://api.twelvedata.com/time_series?symbol=${TWELVE_SYMBOL}&interval=${tdInterval}&outputsize=${count}&apikey=${TWELVE_DATA_KEY}`);
  const d = await r.json();
  if (!d.values) throw new Error('Twelve Data candles error: ' + JSON.stringify(d));
...
app.get('/api/signal', (req, res) => {
  ...
  if (age > 3) return res.status(404).json({ error: 'Signal stale' });
  res.json(lastSignal);
});
