...
// Yahoo Finance — no API key, no limits, correct spot gold price
const YF_SYMBOL = 'XAUUSD%3DX';
...
async function fetchPrice() {
  const r = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${YF_SYMBOL}?interval=1m&range=1d`);
  const d = await r.json();
  const price = d?.chart?.result?.[0]?.meta?.regularMarketPrice;
  if (price) return parseFloat(price);
  throw new Error('Yahoo price failed');
}

async function fetchCandles(interval, count) {
  const yfInterval = { '15m':'15m','1h':'1h','4h':'1h','1d':'1d' }[interval] || '1h';
  const rangeMap   = { '15m':'5d','1h':'1mo','4h':'1mo','1d':'3mo' }[interval] || '1mo';
  const r = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${YF_SYMBOL}?interval=${yfInterval}&range=${rangeMap}`);
  const d = await r.json();
  const result = d?.chart?.result?.[0];
  if (!result) throw new Error('Yahoo candles failed');
