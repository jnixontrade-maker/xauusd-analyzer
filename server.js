...
async function fetchPrice() {
  ...
  throw new Error('Price fetch failed: ' + JSON.stringify(d));
}

async function fetchCandles(interval, count) {
  const tdInterval = { '1m':'1min','5m':'5min','15m':'15min','1h':'1h','4h':'4h','1d':'1day' }[interval] || '1h';
  ...
  const d = await r.json();
  if (!d.values) throw new Error('Candle fetch failed: ' + JSON.stringify(d));
...
async function runAnalysis() {
  try {
    ...
    const [c1m, c5m, c15m, c1h, c4h, price] = await Promise.all([
      fetchCandles('1m',  30),
      fetchCandles('5m',  20),
      fetchCandles('15m', 50),
      fetchCandles('1h',  50),
